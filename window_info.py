...
class WindowInfo(Toplevel):
    def __init__(self, bulbList, parent):
        ...
        # Dodaje zerowa koluimne wymaganą przez tkinter
        tree.column(f"#0", anchor=tk.CENTER, width=0)
        tree.heading(f"#0", text=f"")
        # nagłówki kolumn
        for i, j in enumerate(DEFAULT_PROPS):
            tree.column(f"#{i + 1}", anchor=tk.CENTER, width=80)
            tree.heading(f"#{i + 1}", text=f"{j}")

        tree.grid(row=0, column=0, sticky='ns')

        # add a scrollbar
        scrollbar_vertical = ttk.Scrollbar(self, orient=tk.VERTICAL, command=tree.yview)
